...
def main(root, device, doPca=False, doMinMax=False):
    ...
    lambda_gp = 10
    datapath = root
    genpath = os.path.join("../OutputFiles/WGAN", "gendata")
    losspath = os.path.join("../OutputFiles/WGAN", "LossDetails")
...
# %%
allfiles = sorted(glob.glob(os.path.join("../InputFiles", "Group*Data.csv")))
print(allfiles)
...
allfiles = sorted(glob.glob(os.path.join("../InputFiles", "Group*Axes.csv")))
print(allfiles)
allfiles = [allfiles[0], allfiles[1]]
print(allfiles)
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
