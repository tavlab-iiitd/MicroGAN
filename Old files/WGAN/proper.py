...
def main(root):
    ...
    genfiles = glob.glob(os.path.join(root, "gendata", "data*"))
    gendict = {}
    for i in genfiles:
        df = pd.read_csv(i)
        df.drop("Unnamed: 0", axis=1, inplace=True)
        df.reset_index(drop=True, inplace=True)
        if i.split("/")[-1][4] not in gendict.keys():
            gendict[i.split("/")[-1][4]] = df
        else:
            gendict[i.split("/")[-1][4]] = pd.concat([gendict[i.split("/")[-1][4]], df], axis=0, ignore_index=True)
