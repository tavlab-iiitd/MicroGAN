...
def plot_tsne(gendata, actual, name, labels=6, plotpath=None):
    ...
    filename = os.path.join(plotpath, "tsne_plot_")
...
def genmain(root):
    ...
    df.drop(["Unnamed: 0", "tuberculosis", "hiv"], axis=1, inplace=True)
    ...
    labels = [0, 1, 2, 3, 4, 5]
    for i in labels:
        dicti[i] = df[df['label'] == i]
        dicti[i].reset_index(inplace=True, drop=True)
